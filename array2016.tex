...
\begin{abstract}
We present Ikra, an array-based parallel extension to Ruby with a just-in-time compiler. The high-level goal of Ikra is to allow developers to exploit GPU-based high-performance computing without having to be familiar with intricate details of the underlying GPU infrastructure and CUDA. %researchers, who are not familiar with CUDA and GPU-specific architectural details, to take advantage of GPU-based high-perforResearch
...
Benchmark with/without job reordering and with/without columnar object layout (4 benchmarks)
